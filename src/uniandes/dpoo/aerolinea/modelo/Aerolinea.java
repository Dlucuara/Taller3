...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	...
    	for(Vuelo V: vuelos) {
    		String codigo = V.getRuta().getCodigoRuta();
    		if(codigo.equals(codigoRuta)&& V.getFecha().equals(fechaVuelo)) {
    			return V;
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        for (Vuelo v : vuelos) {
            if (v.getAvion().equals(avionSeleccionado) && v.getFecha().equals(fecha)) {
                throw new InformacionInconsistenteException("El avión ya tiene un vuelo en esa fecha.");
            }
        }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (cliente == null) {
            throw new InformacionInconsistenteException("Cliente no encontrado: " + identificadorCliente);
        }

        if (vuelo == null) {
            throw new InformacionInconsistenteException("Vuelo no encontrado para ruta " + codigoRuta + " en fecha " + fecha);
        }
        
    	int capacidad = vuelo.getAvion().getCapacidad();
    	int cantidadTiquetes = vuelo.getTiquetes().size();
    	int espacioDisponible = capacidad - cantidadTiquetes;
        ...
        for (int i = 0; i < cantidad; i++) {
            total += calculadora.calcularTarifa(vuelo, cliente);
        }
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
    	...
    	for(Vuelo v: vuelos) {
    		Collection<Tiquete> tiquetes = v.getTiquetes();
    		
    		for(Tiquete t: tiquetes) {
    			if(t.getCliente().getIdentificador().equals(cliente.getIdentificador())&& !t.esUsado()) {
    				saldoPendiente += t.getTarifa();
    	...
    	return Integer.toString(saldoPendiente);
    }
